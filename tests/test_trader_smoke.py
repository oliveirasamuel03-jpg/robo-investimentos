...
def test_normalize_twelvedata_interval_maps_provider_specific_aliases(isolated_storage):
    ...
    assert market_data._normalize_twelvedata_interval("1m") == "1min"
    assert market_data._normalize_twelvedata_interval("5m") == "5min"
    assert market_data._normalize_twelvedata_interval("15m") == "15min"
    assert market_data._normalize_twelvedata_interval("30m") == "30min"
    assert market_data._normalize_twelvedata_interval("60m") == "1h"
...
def test_fetch_market_data_map_normalizes_intraday_interval_for_twelvedata_chart_request(isolated_storage, monkeypatch):
    market_data = load_module("core.market_data")
    ...
    def fake_urlopen(request, timeout=20):
        ...
        return _FakeUrlopenResponse(
            {
                "status": "ok",
                "meta": {"symbol": "BTC/USD", "interval": "15min", "type": "Digital Currency"},
                "values": [
                    {
                        "datetime": "2026-04-22 00:00:00",
                        "open": "100.0",
    ...
    assert "interval=15min" in captured["url"]
    assert result.status["requested_interval"] == "15m"
    assert result.status["effective_interval"] == "15min"
    assert result.status["provider"] == "twelvedata"
    assert result.status["feed_status"] == "LIVE"
