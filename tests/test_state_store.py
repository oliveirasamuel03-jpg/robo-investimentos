...
def test_state_store_bootstraps_files_and_defaults(isolated_storage):
    ...
    assert state["multi_timeframe_swing_audit"]["estimated_provider_calls"] == 0
    assert state["bos_pivot_trace_audit"]["enabled"] == config.BOS_PIVOT_TRACE_AUDIT_ENABLED
    ...
    assert state["reversal_blocker_routing_audit"]["mode"] == "DIAGNOSTIC_ONLY"
    assert state["reversal_blocker_routing_audit"]["safety_mode"] == "SHADOW_ONLY"
    ...
    assert state["bos_confirmation_quality_audit"]["mode"] == "DIAGNOSTIC_ONLY"
    assert state["bos_confirmation_quality_audit"]["safety_mode"] == "SHADOW_ONLY"
    assert state["bos_confirmation_quality_audit"]["should_keep_blocked"] is True
    ...
    assert state["post_10d_calibration_plan"]["mode"] == "PLANNING_ONLY"
    assert state["post_10d_calibration_plan"]["diagnostic_mode"] == "DIAGNOSTIC_ONLY"
    assert state["post_10d_calibration_plan"]["safety_mode"] == "SHADOW_ONLY"
    assert state["post_10d_calibration_plan"]["plan_status"] == "INSUFFICIENT_DATA_FOR_PLAN"
    assert state["post_10d_calibration_plan"]["should_continue_paper"] is True
    assert state["post_10d_calibration_plan"]["should_start_real_money"] is False
    ...
    assert state["phase2_1_fine_tune"]["phase2_1_fine_tune_enabled"] is True
    assert state["phase2_1_fine_tune"]["phase2_1_fine_tune_target"] == "trend_pullback_breakout_multi_minor_confirmation"
    assert state["phase2_1_fine_tune"]["phase2_1_fine_tune_applied_count"] == 0
    assert state["phase2_1_fine_tune"]["phase2_1_fine_tune_blocked_count"] == 0
