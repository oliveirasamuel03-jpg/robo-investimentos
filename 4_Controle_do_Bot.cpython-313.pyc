...
def plot_equity(backtest_result, title: str):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=backtest_result["equity_curve"].index, y=backtest_result["equity_curve"]["equity"], mode="lines", name=title))
...
with c4:
    fast_mode = st.toggle("Modo rápido", True)
with c5:
    delay = st.slider("Atraso de execução", 1, 3, 1, 1)
with c6:
...
with a1:
    include_brazil_stocks = st.checkbox("Ações Brasil", value=bool(state.get("include_brazil_stocks", True)))
    include_us_stocks = st.checkbox("Ações EUA", value=bool(state.get("include_us_stocks", True)))
with a2:
...
with a3:
    ...
    include_grains = st.checkbox("Grãos", value=bool(state.get("include_grains", True)))
...
if st.button("Salvar parâmetros e rodar pesquisa", use_container_width=True):
    ...
    state["investment"]["last_report"] = result["report"]
    save_bot_state(state)
    append_csv_row(INVESTOR_ORDERS_FILE, {"timestamp": pd.Timestamp.utcnow().isoformat(), "metric": "research_run", "value": len(result['prices_columns']), "notes": "Pesquisa institucional executada"})
    ...
    st.plotly_chart(px.histogram(result["mc"]["simulations"], x="sharpe", title="Distribuição de Sharpe - Monte Carlo"), use_container_width=True)
    with st.expander("Relatório institucional"):
        st.json(result["report"])
else:
    last_report = state.get("investment", {}).get("last_report", {})
    if last_report:
        st.info("Último relatório salvo em estado local.")
        st.json(last_report)
