...
def update_runtime_state(last_action: str, next_run_delta_seconds: int) -> None:
    ...
    state["next_run_at"] = (datetime.now(timezone.utc) + timedelta(seconds=next_run_delta_seconds)).isoformat()
...
def mark_error(message: str) -> None:
    ...
    state["last_run_at"] = utc_now_iso()
    state["next_run_at"] = (datetime.now(timezone.utc) + timedelta(seconds=SLEEP_SECONDS)).isoformat()
