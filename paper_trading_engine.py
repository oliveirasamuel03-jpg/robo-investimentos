...
def run_paper_cycle(config: PaperTradingConfig | None = None) -> dict:
    ...
    prices = load_data(
        ...
        include_brazil_stocks=config.include_brazil_stocks,
        include_us_stocks=config.include_us_stocks,
        include_etfs=config.include_etfs,
        include_fiis=config.include_fiis,
        include_crypto=config.include_crypto,
        include_grains=config.include_grains,
...
def build_paper_report(initial_capital: float | None = None) -> dict:
    ...
    if initial_capital is None:
        initial_capital = _safe_float(state.get("config", {}).get("initial_capital", 10000.0), 10000.0)
