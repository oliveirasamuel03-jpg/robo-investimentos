...
with c1:
    ticket = st.number_input("Valor por operação (R$)", min_value=MIN_TICKET, max_value=MAX_TICKET, value=float(trader["ticket_value"]), step=10.0)
with c2:
    holding = st.slider("Holding máximo (min)", min_value=MIN_HOLDING_MINUTES, max_value=MAX_HOLDING_MINUTES, value=int(trader["holding_minutes"]), step=1)
with c3:
    max_open = st.slider("Máx. posições abertas", min_value=1, max_value=20, value=int(trader["max_open_positions"]), step=1)

watchlist_text = st.text_input("Watchlist do Trader", value=", ".join(trader.get("watchlist", [])))
...
with col_b:
    if st.button("Rodar ciclo Trader", use_container_width=True):
        ...
        st.success(f"Ciclo do trader executado. Trades: {result.get('cycle_result', {}).get('trades_executed', 0)}")
with col_c:
