...
def _setup_blocker_taxonomy_audit_summary(validation_report: dict[str, Any]) -> str:
    audit = dict(validation_report.get("setup_blocker_taxonomy_audit", {}) or {})
    if not audit:
        return "Setup/blocker taxonomy audit: no taxonomy sample yet. Diagnostic only; no trade decision changed."
    if not bool(audit.get("enabled", True)):
        return "Setup/blocker taxonomy audit: disabled. Diagnostic only; no trade decision changed."
    return (
        "Setup/blocker taxonomy audit: "
        f"top={_safe_text(audit.get('top_symbol'), fallback='none')}; "
        f"official={_safe_text(audit.get('official_primary_blocker'), fallback='none')}; "
        f"normalized={_safe_text(audit.get('normalized_primary_reason'), fallback='UNKNOWN')}; "
        f"status={_safe_text(audit.get('taxonomy_status'), fallback='INSUFFICIENT_DATA_FOR_TAXONOMY')}; "
        f"feed_clean={str(bool(audit.get('current_feed_is_clean', False))).lower()}; "
        f"recommendation={_safe_text(audit.get('recommendation'), fallback='insufficient_data')}. "
        "Diagnostic only; no trade decision changed."
    )
...
def _h1_confirmation_after_h4_bos_audit_summary(validation_report: dict[str, Any]) -> str:
    audit = dict(validation_report.get("h1_confirmation_after_h4_bos_audit", {}) or {})
    if not audit:
        return "H1 confirmation after H4 BOS audit: no H1/H4 sample yet. Diagnostic only; no trade decision changed."
    if not bool(audit.get("enabled", True)):
        return "H1 confirmation after H4 BOS audit: disabled. Diagnostic only; no trade decision changed."
    return (
        "H1 confirmation after H4 BOS audit: "
        f"top={_safe_text(audit.get('top_symbol'), fallback='none')}; "
        f"h4_bos={_safe_text(audit.get('h4_bos_state'), fallback='INSUFFICIENT_DATA')}; "
        f"h1_bos={_safe_text(audit.get('h1_bos_state'), fallback='INSUFFICIENT_DATA')}; "
        f"status={_safe_text(audit.get('h1_confirmation_status'), fallback='INSUFFICIENT_DATA_FOR_H1_CONFIRMATION')}; "
        f"recommendation={_safe_text(audit.get('recommendation'), fallback='insufficient_data')}. "
        "Diagnostic only; no trade decision changed."
    )
...
def _market_structure_audit_summary(validation_report: dict[str, Any]) -> str:
    audit = dict(validation_report.get("market_structure_audit", {}) or {})
    if not audit:
        return "Market structure/Fibonacci audit: no structural sample yet. Shadow only."
    if not bool(audit.get("market_structure_audit_enabled", True)):
        return "Market structure/Fibonacci audit: disabled. Shadow only."
    top_symbol = _safe_text(audit.get("market_structure_top_symbol"), fallback="none")
    top_score = audit.get("market_structure_top_score")
    top_score_text = "-" if top_score is None else f"{float(top_score):.2f}"
    fib_zone = _safe_text(audit.get("market_structure_top_zone"), fallback="none")
    candidates = int(audit.get("market_structure_candidates_count", 0) or 0)
    recommendation = _safe_text(audit.get("market_structure_top_recommendation"), fallback="sem dados suficientes")
    return (
        f"Market structure/Fibonacci audit: top={top_symbol}; score={top_score_text}; "
        f"fib_zone={fib_zone}; shadow_candidates={candidates}; recommendation={recommendation}. "
        "Shadow only; no trade decision, score, broker, or threshold was changed."
    )


def _fib_alignment_audit_summary(validation_report: dict[str, Any]) -> str:
    audit = dict(validation_report.get("fib_alignment_audit", {}) or {})
    if not audit:
        return "Fibonacci video/PDF alignment audit: no comparison sample yet. Shadow only."
    if not bool(audit.get("fib_alignment_enabled", True)):
        return "Fibonacci video/PDF alignment audit: disabled. Shadow only."
    symbol = _safe_text(audit.get("fib_alignment_top_symbol"), fallback="none")
    score = audit.get("fib_alignment_score")
    score_text = "-" if score is None else f"{float(score):.2f}"
    status = _safe_text(audit.get("fib_alignment_status"), fallback="insufficient_data")
    why_differs = _safe_text(audit.get("fib_alignment_why_differs"), fallback="no divergence note")
    recommendation = _safe_text(audit.get("fib_alignment_recommendation"), fallback="keep_shadow_only")
    return (
        f"Fibonacci video/PDF alignment audit: symbol={symbol}; score={score_text}; "
        f"status={status}; divergence={why_differs}; recommendation={recommendation}. "
        "Shadow only; this measures objective checklist adherence and does not change trades, score, broker, or thresholds."
    )
