...
MACRO_ALERTS_ENABLED = env_flag("MACRO_ALERTS_ENABLED", False)
MACRO_ALERT_PRE_EVENT_MINUTES = max(0, env_int("MACRO_ALERT_PRE_EVENT_MINUTES", 60))
MACRO_ALERT_EVENT_WINDOW_MINUTES = max(1, env_int("MACRO_ALERT_EVENT_WINDOW_MINUTES", 30))
MACRO_ALERT_POST_EVENT_MINUTES = max(0, env_int("MACRO_ALERT_POST_EVENT_MINUTES", 60))
MACRO_ALERTS_FILE = _env_str("MACRO_ALERTS_FILE", "")
EXTERNAL_SIGNAL_WEBHOOK_ENABLED = env_flag("EXTERNAL_SIGNAL_WEBHOOK_ENABLED", False)
EXTERNAL_SIGNAL_SECRET = _env_str("EXTERNAL_SIGNAL_SECRET", "")
EXTERNAL_SIGNAL_MAX_AGE_SECONDS = max(1, env_int("EXTERNAL_SIGNAL_MAX_AGE_SECONDS", 300))
EXTERNAL_SIGNAL_ALLOWED_SOURCES = _env_str("EXTERNAL_SIGNAL_ALLOWED_SOURCES", "")
EXTERNAL_SIGNAL_ALLOWED_TIMEFRAMES = (
    _env_str("EXTERNAL_SIGNAL_ALLOWED_TIMEFRAMES", "30s,1m,5m,15m,1h,1d")
    or "30s,1m,5m,15m,1h,1d"
)
EXTERNAL_SIGNAL_DEDUPE_SECONDS = max(1, env_int("EXTERNAL_SIGNAL_DEDUPE_SECONDS", 300))
EXTERNAL_SIGNAL_TEST_PANEL_ENABLED = env_flag("EXTERNAL_SIGNAL_TEST_PANEL_ENABLED", False)
CALIBRATION_PREVIEW_ENABLED = env_flag("CALIBRATION_PREVIEW_ENABLED", True)
CALIBRATION_PREVIEW_MARGIN = min(0.10, max(0.0, env_float("CALIBRATION_PREVIEW_MARGIN", 0.04)))
CALIBRATION_PREVIEW_MAX_EXAMPLES = min(20, max(1, env_int("CALIBRATION_PREVIEW_MAX_EXAMPLES", 10)))
MULTITF_SWING_AUDIT_ENABLED = env_flag("MULTITF_SWING_AUDIT_ENABLED", True)
MULTITF_SWING_TIMEFRAMES = _env_str("MULTITF_SWING_TIMEFRAMES", "1d,4h,1h") or "1d,4h,1h"
MULTITF_SWING_CACHE_TTL_SECONDS = max(300, env_int("MULTITF_SWING_CACHE_TTL_SECONDS", 900))
MULTITF_SWING_MAX_SYMBOLS = min(5, max(1, env_int("MULTITF_SWING_MAX_SYMBOLS", 5)))
MULTITF_SWING_REQUIRE_LIVE_FEED = env_flag("MULTITF_SWING_REQUIRE_LIVE_FEED", True)
# Non-negotiable safety: FASE 2.5 can be disabled, but it cannot become trade authority.
MULTITF_SWING_SHADOW_ONLY = True
MULTITF_INTRADAY_FETCH_ENABLED = env_flag("MULTITF_INTRADAY_FETCH_ENABLED", True)
MULTITF_INTRADAY_TIMEFRAMES = _env_str("MULTITF_INTRADAY_TIMEFRAMES", "4h,1h") or "4h,1h"
MULTITF_INTRADAY_CACHE_TTL_SECONDS = max(900, env_int("MULTITF_INTRADAY_CACHE_TTL_SECONDS", 1800))
MULTITF_INTRADAY_MAX_SYMBOLS = min(5, max(1, env_int("MULTITF_INTRADAY_MAX_SYMBOLS", 5)))
MULTITF_INTRADAY_MAX_CALLS_PER_CYCLE = min(10, max(0, env_int("MULTITF_INTRADAY_MAX_CALLS_PER_CYCLE", 3)))
MULTITF_INTRADAY_REQUIRE_LIVE_FEED = env_flag("MULTITF_INTRADAY_REQUIRE_LIVE_FEED", True)
